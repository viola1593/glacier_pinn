...
class Mass_conservation_loss(torch.nn.Module):
    ...
    """
    def __init__(self, weight: float=1.0, x_idx=0, vel_x_idx=1, h_idx=0, depth_vel_x_idx=1, h_true_idx=0, mb_idx=1) -> None:
        super().__init__()
        self.weight = weight
...
class Thickness_smoothing_loss(torch.nn.Module):
    ...
    def __init__(self, weight: float=1.0, x_idx=0, h_idx=0,  h_true_idx=0) -> None:
        super().__init__()
        self.weight = weight
        # set the indices of the model input, output, and target components, can be changed if the order of the inputs or outputs changes.
    ...
    def forward(self, model_output, model_input, input_std, target_std):
        h_x = torch.autograd.grad(model_output[:,self.h_idx], model_input, 
                                          grad_outputs=torch.ones_like(model_output[:,self.h_idx]).type_as(model_input), 
                                          create_graph=True, retain_graph=True)[0][:,self.x_idx]*target_std[self.h_true_idx]/input_std[self.x_idx]
        return (torch.mean(torch.square(h_x)))*self.weight
...
class DepthAvgVel_loss(torch.nn.Module):
    ...
    """
    def __init__(self, vel_lowerbound: float=0.7, weight: float=1.0, vel_x_idx=1, depth_vel_x_idx=1, h_true_idx=0, beta_x_idx=3, ) -> None:
        super().__init__()
        self.weight = weight
    ...
    def forward(self, pred, model_input, input_std, input_mean):
        ...
        vel_x_loss = velocity_loss_2(vx_surface, vx_avg, self.lower,  beta_x)
        
        
        velocity_data_loss = torch.mean(vel_x_loss)
...
class DepthAvgVelMag_loss(torch.nn.Module):
    ...
    def __init__(self, vel_lowerbound: float=0.7, weight: float=1.0, vel_x_idx=1, depth_vel_x_idx=1,  beta_mag_idx=3) -> None:
        super().__init__()
        self.lower = vel_lowerbound
        self.weight = weight
        self.vel_x_idx = vel_x_idx
    ...
    def forward(self, pred, model_input, input_std, input_mean):
        ...
        pred_mag = torch.sqrt(torch.abs(vx_avg))
        obs_mag = torch.sqrt(torch.square(vx_surface))
...
def velocity_loss_2(v_surf, v_d, lower_bound, beta):
    """
